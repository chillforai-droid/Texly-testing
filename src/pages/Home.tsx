...
const HomePage = () => {
  ...
  return (
    <main className="relative min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      {/* Background Decoration — desktop only, mobile पर GPU crash करता है इसलिए hidden */}
      <div className="hidden sm:block absolute inset-0 overflow-hidden pointer-events-none -z-10" style={{ isolation: 'isolate' }}>
        <div className="absolute -top-[10%] -left-[10%] w-[40%] h-[40%] bg-blue-500/5 dark:bg-blue-500/8 blur-[60px] rounded-full" />
        <div className="absolute top-[20%] -right-[5%] w-[30%] h-[30%] bg-indigo-500/5 dark:bg-indigo-500/8 blur-[50px] rounded-full" />
        <div className="absolute bottom-[10%] left-[20%] w-[25%] h-[25%] bg-emerald-500/5 dark:bg-emerald-500/8 blur-[50px] rounded-full" />
      </div>
      ...
      <section className="py-12 relative">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Link
              to="/ai-automation"
              className="group flex items-start gap-4 p-5 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl hover:border-slate-400 hover:shadow-lg hover:shadow-slate-500/10 transition-all"
            >
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-700 to-slate-900 flex items-center justify-center shrink-0 text-white font-black text-lg">
                🐙
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-black text-slate-600 uppercase tracking-wide">Free</span>
                  <span className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-full font-bold">⚡ Fast</span>
                </div>
                <h3 className="font-black text-slate-900 dark:text-white text-sm leading-snug group-hover:text-slate-600 transition-colors">
                  Free GitHub File Push Tool
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-2">
                  Git install के बिना browser से GitHub repo में files push करें। 100% free।
                </p>
              </div>
            </Link>
          </div>
...
{/* AI Tools Section */}
      <section className="py-20 relative" style={{ isolation: 'isolate' }}>
        <div className="hidden sm:block absolute top-0 left-0 right-0 h-80 bg-blue-600/5 dark:bg-blue-600/8 blur-[60px] rounded-full pointer-events-none mx-auto max-w-2xl -z-10" />
        ...
        {/* Ad Slot 1 — uncomment when real AdSense slot IDs are configured */}
      ...
      {!searchQuery && (
        ...
        {/* Ad Slot 2 — uncomment when real AdSense slot IDs are configured */}
