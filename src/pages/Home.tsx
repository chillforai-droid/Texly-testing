      ...
      <section className="py-12 relative">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {ALL_TOOLS.filter(t => ['whatsapp-text-formatter', 'number-to-words', 'site-guard'].includes(t.id)).map((tool) => (
              <Link
                key={tool.id}
                to={tool.id === 'site-guard' ? `/tools/${tool.slug}` : `/tool/${tool.slug}`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {tool.id === 'site-guard' && (
                      <span className="text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 px-2 py-0.5 rounded-full font-bold">🛡️ AI Tool</span>
                    )}
                  </div>
