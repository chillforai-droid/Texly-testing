...
import { 
  ...
  Eye, EyeOff, Settings2, Layers, Zap
} from 'lucide-react';
    ...
    try {
      ...
      for (let i = 0; i < acceptedFiles.length; i++) {
        ...
        if (file.type === 'application/pdf') {
          ...
          // For password-remover tool, try loading with provided password first
          // For other tools, try without password
          const loadOptions: Record<string, unknown> = { data: arrayBuffer };
          if (toolId === 'pdf-password-remover' && password) {
          ...
          try {
            ...
            const errName = (loadErr as { name?: string })?.name ?? '';
            const errMsg = String((loadErr as { message?: string })?.message ?? '').toLowerCase();
            const isPasswordErr = errName === 'PasswordException' || errMsg.includes('password');

            if (isPasswordErr) {
              // PDF is password-protected — don't show error, just skip thumbnail loading
              // The file is still stored in `files` state and will be used during process
              if (toolId === 'pdf-password-remover') {
                setIsPasswordProtected(true);
                setError(null); // No error — this is expected behaviour for this tool
              } else {
                setError('This PDF is password-protected. Please use the "PDF Password Remover" tool to unlock it first.');
              }
              continue; // skip thumbnail generation for this file, but keep it
            }
            throw loadErr; // re-throw non-password errors
          }
      ...
      setError('Failed to load some PDF pages. Please try again.');
    } finally {
  ...
  return (
    <div className="space-y-8">
      ...
      {(toolId === 'pdf-password-remover' || toolId === 'generate-pdf' || toolId === 'pdf-editor') && !result && (
        <div className="p-4 sm:p-6 bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2 col-span-full">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && files.length > 0 && !isProcessing && document.getElementById('pdf-process-btn')?.click()}
                    placeholder="Enter PDF password to unlock..."
                    className="w-full pl-12 pr-12 py-3 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all"
                  />
                  <button
                    type="button"
                ...
                <p className="text-xs text-slate-500">Leave blank if the PDF has no open password (owner/print restrictions only).</p>
                {files.length > 0 && (
                  <button
                    type="button"
                    onClick={() => {
                      const filesToReload = [...files];
                      setPdfPages([]);
                      setTimeout(() => onDrop(filesToReload), 50);
                    }}
                    className="mt-1 flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-colors"
                  >
      ...
      {!result && toolId !== 'generate-pdf' && (
        ...
        >
          ...
          <div className="hidden sm:block absolute top-0 right-0 w-64 h-64 bg-blue-500/5 blur-[100px] rounded-full -mr-32 -mt-32"></div>
          <div className="hidden sm:block absolute bottom-0 left-0 w-64 h-64 bg-indigo-500/5 blur-[100px] rounded-full -ml-32 -mb-32"></div>
        </div>
      )}
        ...
        <div className="space-y-8">
          ...
          {isLoadingPages ? (
            ...
            <Reorder.Group 
              ...
              className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-6"
            >
              {pdfPages.map((page) => (
                <Reorder.Item
                  key={page.id}
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              id="pdf-process-btn" style={{minHeight: "48px"}}
...
async function removePdfPassword(file: File, pass: string): Promise<Blob> {
  ...
  // Step 1: Use pdfjs-dist to open the PDF with the password.
  // pdf-lib cannot decrypt encrypted PDFs at all, so we must use pdfjs here.
  let pdf: any;
  try {
    ...
    if (e && (e.name === 'PasswordException' || String(e.message).toLowerCase().includes('password'))) {
      throw new Error(pass ? 'Incorrect password. Please try again.' : 'This PDF is password-protected. Please enter the password.');
    }
    throw new Error('Failed to open PDF. The file may be corrupted.');
  }

  // Step 2: Render each page to a canvas and collect as PNG data URLs.
  // Scale = 2.0 for good quality (≈150dpi equivalent)
  const scale = 2.0;
  ...
  // Step 3: Build a brand-new, unencrypted PDF using pdf-lib.
  // Each page is the canvas PNG embedded as an image — no password required.
  const newPdf = await PDFDocument.create();

  for (const { dataUrl, width, height } of pageImages) {
    ...
    const pngImage = await newPdf.embedPng(bytes);
    const page = newPdf.addPage([width / scale, height / scale]);
