...
interface RatingSystemProps {
  toolId: string;
  theme: any;
  onRatingLoad?: (avgRating: number, totalCount: number) => void;
}

const RatingSystem: React.FC<RatingSystemProps> = ({ toolId, theme, onRatingLoad }) => {
  ...
  const fetchRatings = async () => {
    if (!supabase) {
      ...
      const avg = parseFloat((sum / storedRatings.length).toFixed(1));
      const count = storedRatings.length;
      setAverageRating(avg);
      setTotalRatings(count);
      setLoading(false);
      onRatingLoad?.(avg, count);
      return;
    }
